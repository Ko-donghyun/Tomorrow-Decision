...
public class ThemeWidgetProvider extends AppWidgetProvider {
    ...
    public ArrayList<TodoItem> settingTodo(Context context, Long currentTime) {
        ...
        Cursor cursor = sqLiteDatabase.rawQuery("SELECT * FROM todo WHERE time IN (?, ?, ?, ?, ?)",  new String[] {
                ...
                Long.toString((currentTime / 3600000) * 3600000 + 3600000),
                Long.toString((currentTime / 3600000) * 3600000 + 7200000),
        });
    ...
    public String[] settingThemeColor(Context context) {

        String[] importanceColorCodes = new String[10];

        for (int i = 0; i < 5; i++) {
            importanceColorPreferences[i * 2] = context.getSharedPreferences("level_" + Integer.toString(i) + "_tg", Activity.MODE_PRIVATE);
            importanceColorPreferences[(i * 2) + 1] = context.getSharedPreferences("level_" + Integer.toString(i) + "_bg", Activity.MODE_PRIVATE);
            importanceColorCodes[(i * 2)] = importanceColorPreferences[(i * 2)].getString("level_" + Integer.toString(i) + "_tg", "#000000");
            importanceColorCodes[(i * 2) + 1] = importanceColorPreferences[(i * 2) + 1].getString("level_" + Integer.toString(i) + "_bg", "#FFFFFF");
        }

        return importanceColorCodes;
    }

    public void settingTodoView(Context context, AppWidgetManager appWidgetManager, int appWidgetId, ArrayList<TodoItem> listViewItemList, String[] importanceColorCodes) {
        // RemoteViews를 이용해 Text설정
        RemoteViews updateViews = new RemoteViews(context.getPackageName(), R.layout.theme_widget_layout_2by2);
        ...
        updateViews.setTextColor(R.id.time_text_2, Color.parseColor(importanceColorCodes[(listViewItemList.get(1).getImportance() * 2)]));
        updateViews.setTextViewText(R.id.todo_item_2, listViewItemList.get(1).getTodo());
        updateViews.setTextColor(R.id.todo_item_2, Color.parseColor(importanceColorCodes[(listViewItemList.get(1).getImportance() * 2)]));
        updateViews.setInt(R.id.item_layout_2, "setBackgroundColor", Color.parseColor(importanceColorCodes[(listViewItemList.get(1).getImportance() * 2) + 1]));

        updateViews.setTextViewText(R.id.todo_item_3, listViewItemList.get(2).getTodo());
        updateViews.setTextColor(R.id.todo_item_3, Color.parseColor(importanceColorCodes[(listViewItemList.get(2).getImportance() * 2)]));
        updateViews.setInt(R.id.item_layout_3, "setBackgroundColor", Color.parseColor(importanceColorCodes[(listViewItemList.get(2).getImportance() * 2) + 1]));

        updateViews.setTextViewText(R.id.todo_item_4, listViewItemList.get(3).getTodo());
        updateViews.setTextColor(R.id.todo_item_4, Color.parseColor(importanceColorCodes[(listViewItemList.get(3).getImportance() * 2)]));
        updateViews.setInt(R.id.item_layout_4, "setBackgroundColor", Color.parseColor(importanceColorCodes[(listViewItemList.get(3).getImportance() * 2) + 1]));
    ...
    public void updateWidgetAll(Context context, AppWidgetManager appWidgetManager, int[] appWidgetIds) {
        ...
        // 중요도 컬러 가져오기
        String[] importanceColorCodes = settingThemeColor(context);

        // 뷰에 반영
        for (int i = 0; i < appWidgetIds.length; i++) {
            settingTodoView(context, appWidgetManager, appWidgetIds[i], listViewItemList, importanceColorCodes);
        }
    }
}